...
## Shift 81 — Петрович — checkpoint
- Independent OTK finalized shift 80 APPROVED 8/10 after verifying runtime loss, target diff and the evidence-only nature of commit `f56c1d563f73c2621b4e3a5dac95330741d2b98c`.
- Immutable shift-81 start report commit `60a55b441bcd325e19bb39ee36f9b88f9d2842e2` passed exact Agent Runtime Check run `35483006855` SUCCESS.
- Consumed already-terminal verification for target `f56c1d563f73c2621b4e3a5dac95330741d2b98c`: Ramdisk Tool Windows run `35482564500` SUCCESS; Windows Build run `35482564537` SUCCESS; exact Windows End-to-End Boot run `35482564441` FAILURE.
- Downloaded exact E2E artifact `ios-darwin-windows-e2e` and read `apfs-structural-evidence.json`. Active descriptor-ring scan confirms the block-0 values were not stale: source latest checkpoint is XID 9 / nextXID 10 at block 43214; rebuilt latest checkpoint is XID 1 / nextXID 2 at block 2. Boot still fails APFS mountroot with errno 79.
- Pinned upstream writer evidence changes interpretation: `apfswrite` intentionally defines `formatXID = 1` because it writes a single static checkpoint; XID 1 is therefore writer architecture, not by itself proof of corruption. Blindly forcing source XID 9 would require changing object/checkpoint transaction semantics across the writer and is not justified.
- Nearest discriminating evidence is now volume-superblock (APSB) semantics rather than another writer mutation: compare source/rebuilt FSIndex, compatible/read-only/incompatible feature flags, meta-crypto state, tree types, volume flags, role/group, root/revert/snapshot metadata fields. The pinned reader exposes these fields through `Volume.Superblock`; no speculative writer change should precede that comparison.
